    ...
    <v-container fluid v-class:phone-layout="$vuetify.breakpoint.xsOnly">
        <v-layout column v-show="hasSelected" class="toolbar">
            <v-toolbar flat dense color="white" height="56px">
                ...
                <v-divider vertical inset/>
                <v-btn icon @click="showInfo()" title="信息" v-if="selectedRows.length <= 4">
                ...
                <v-menu offset-y>
                    ...
                    <v-list class="auto-actions">
                        ...
                        >
                            ...
                            <v-list-tile-content>
                                <v-list-tile-title>{{ item?'开启':'关闭' }}</v-list-tile-title>
                            </v-list-tile-content>
                        </v-list-tile>
                    </v-list>
                </v-menu>
        ...
        <v-layout column>
            ...
            >
                <template v-slot:items="row">
                    ...
                    <td :title="row.item.name" :class="['icon-label',{auto_tmm:row.item.auto_tmm}]"
                    ...
                    <td>
            <span
                    :title="row.item.added_on | formatTimestamp"
            >{{ row.item.added_on | formatTimestamp }}</span>
                    </td>
                    <td>
            <span
                    :title="row.item.completion_on | formatTimestamp"
            >{{ row.item.completion_on | formatTimestamp }}</span>
                    </td>
                </template>
            </v-data-table>
        </v-layout>
...
<script lang="ts">
    ...
    export default Vue.extend({
        ...
        methods: {
            ...
            async setLocation() {
                const save_path = _.get(this.selectedRows, [0, "save_path"], this.prefs.save_path);
                this.$dialog.prompt({
                    text: `请输入保存路径`,
                    title: "修改保存路径",
                    value: save_path
                }).then(res => {
                    res && api.action("torrents/setLocation", {
            ...
            async editTrackers() {
                this.$dialog.form({
                    ...
                    if (res && res.reg) {
                        const regex = new RegExp(res.reg);
                        const items = _.chain(this.selectedRows)
                            .map(({tracker, hash, name}) => {
                                const newUrl = tracker.replace(regex, res.to || "");
